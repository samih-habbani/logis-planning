...
const TRAINER_STYLE = {
  ali:   { bar: 'border-sky-500/60',    card: 'bg-sky-500/10 border border-sky-500/30 text-sky-200',      name: 'text-sky-400' },
  samih: { bar: 'border-violet-500/60', card: 'bg-violet-500/10 border border-violet-500/30 text-violet-200', name: 'text-violet-400' },
}
const CENTER_BADGE = {
...
export function PlanningCalendar() {
  ...
  const [modal, setModal] = useState<{ open: boolean; date: Date | null; initialStart?: string; event?: PlanningEvent | null }>({ open: false, date: null })
  ...
  async function handleSave(data: EventFormData) {
    ...
    const body = { date: toDateKey(modal.date), trainer: data.trainer, center: data.center, start_time: data.start_time, end_time: data.end_time, curriculum: data.curriculum || null, student_name: data.student_name || null, note: data.note || null }
    if (modal.event) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex items-center gap-3 flex-wrap mb-4 flex-shrink-0">
        ...
        <button onClick={() => { setWeekOffset(0); setActiveMobDay(new Date().getDay() === 0 ? 6 : new Date().getDay() - 1) }}
      ...
      {/* Grid */}
      <div className="flex-1 min-h-0 overflow-auto rounded-xl border border-neutral-800 bg-neutral-900"
        ...
        <div className="grid" style={{ gridTemplateColumns: `52px repeat(${cols}, 1fr)` }}>
          ...
          <div className="border-r border-neutral-800">
            {Array.from({ length: SLOT_COUNT }, (_, s) => {
              const h = H_START + Math.floor((s * SLOT_MINUTES) / 60)
              const m = (s * SLOT_MINUTES) % 60
              return (
            ...
            return (
              <div key={di} className={`relative border-r border-neutral-800 last:border-r-0 ${isWknd ? 'bg-neutral-950/30' : ''}`}>
                {Array.from({ length: SLOT_COUNT }, (_, s) => {
                  ...
                  const m = (s * SLOT_MINUTES) % 60
                  return (
                    <div key={s} style={{ height: SLOT_PX }}
                      className="border-b border-dashed border-neutral-800/50 cursor-pointer hover:bg-white/[0.02] transition-colors group"
                      onClick={() => setModal({ open: true, date: day, initialStart: hhmm(h, m), event: null })}>
                      <span className="hidden group-hover:flex items-center justify-center h-full text-neutral-700 text-base select-none">+</span>
                    </div>
                  )
                })}
      ...
      <EventModal
        open={modal.open} date={modal.date} initialStart={modal.initialStart} event={modal.event}
